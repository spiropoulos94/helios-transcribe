...
import { RefreshCw, ScrollText, Maximize2, Pencil, RotateCcw } from 'lucide-react';
...
interface OfficialMinutesGenerateProps {
...
export default function OfficialMinutesGenerate({
  formState,
  summary,
  isTooLong,
  canGenerate,
    ...
    return (
      <div className="space-y-4">
        ...
        <button
          ...
          className="w-full flex items-center justify-center gap-2 px-4 py-3 text-sm font-medium text-white bg-purple-600 rounded-lg hover:bg-purple-700 transition-colors"
        >
  ...
  if (formState.isGenerating) {
    return (
      <GeneratingSpinner
        title={t.editor?.generating || 'Generating official minutes...'}
        subtitle={t.editor?.generatingDesc || 'This may take a minute for long transcriptions'}
        colorScheme="purple"
      />
    );
  }
    ...
    if (isFullscreen) {
      return (
        <FullscreenEditor
          ...
          title={t.editor?.previewTitle || 'Generated Minutes'}
          t={t}
          colorScheme="purple"
      ...
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <h4 className="font-medium text-slate-900">
            {t.editor?.previewTitle || 'Generated Minutes'}
          </h4>
          <div className="flex items-center gap-2">
            ...
            <DownloadMenu onDownload={onDownload} t={t} colorScheme="purple" />
          </div>
        </div>
        <div className="relative">
          <textarea
            ...
            className="w-full h-[400px] p-4 text-sm text-slate-800 font-mono bg-slate-50 border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-transparent resize-none"
            placeholder={t.editor?.editMinutesPlaceholder || 'Edit the generated minutes here...'}
          />
    ...
    <div className="space-y-4">
      ...
      {isTooLong && (
        <WarningAlert
          ...
          message={t.editor?.longTranscriptionDesc || 'This transcription is quite long. Generation may take longer and the output may be truncated.'}
        />
      )}
      ...
      <button
        ...
        className="w-full flex items-center justify-center gap-2 px-4 py-3 text-sm font-medium text-white bg-purple-600 rounded-lg hover:bg-purple-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
      >
        <ScrollText className="w-4 h-4" />
        {t.editor?.generate || 'Generate Official Minutes'}
      </button>

      <p className="text-xs text-slate-500 text-center">
        {t.editor?.generateNote || 'The AI will transform the transcription into formal municipal council minutes.'}
      </p>
    </div>
  );
}
